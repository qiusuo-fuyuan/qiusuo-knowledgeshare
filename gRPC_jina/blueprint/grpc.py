...
class Jinaer(jina_pb2_grpc.JinaServicer):
    def Reply(self, requst, context):
        ...
        socket.send_string("Please let body pea do some executor work")
        ...
        if message_from_server:
            print("Already received the message from ZMQ Server (HeadPea)")
        # Send the message back to gRPC Client that the mission is completed
        return jina_pb2.JinaReply(message="The mission assigned to pea is completed {request.message}")